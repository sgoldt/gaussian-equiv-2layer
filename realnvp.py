# Utility class implementing the real NVP model.
#
...
class CheckerboardAdditiveCoupling(AbstractCoupling):
    def __init__(self, in_out_dim, mid_dim, size, mask_config, hps, device):
        ...
        self.mask = self.build_mask(size, config=mask_config).to(device)
    ...
    def __init__(self, in_out_dim, mid_dim, size, mask_config, hps, device):
        ...
        if hps.affine:
            self.coupling = CheckerboardAffineCoupling(
                in_out_dim, mid_dim, size, mask_config, hps, device)
        else:
...
class ChannelwiseCoupling(nn.Module):
    def __init__(self, in_out_dim, mid_dim, mask_config, hps, device):
...
class RealNVP(nn.Module):
    def __init__(self, datainfo, prior, hps, device):
        ...
        if datainfo.name == 'cifar10':
            ...
            # SCALE 1: 3 x 32 x 32
            self.s1_ckbd = self.checkerboard_combo(chan, dim, size, hps, device)
            self.s1_chan = self.channelwise_combo(chan*4, dim, hps, device)
            self.order_matrix_1 = self.order_matrix(chan).to(device)
            chan *= 2
            size //= 2

            # SCALE 2: 6 x 16 x 16
            self.s2_ckbd = self.checkerboard_combo(chan, dim, size, hps, device, final=True)
            ...
            self.s1_ckbd = self.checkerboard_combo(chan, dim, size, hps, device)
            self.s1_chan = self.channelwise_combo(chan*4, dim*2, hps, device)
            self.order_matrix_1 = self.order_matrix(chan).to(device)
            chan *= 2
            size //= 2
            dim *= 2
            ...
            self.s2_ckbd = self.checkerboard_combo(chan, dim, size, hps, device)
            self.s2_chan = self.channelwise_combo(chan*4, dim*2, hps, device)
            ...
            self.s3_ckbd = self.checkerboard_combo(chan, dim, size, hps, device)
            self.s3_chan = self.channelwise_combo(chan*4, dim*2, hps, device)
            ...
            if datainfo.name == 'imnet32':
                # SCALE 4: 24 x 4 x 4
                self.s4_ckbd = self.checkerboard_combo(chan, dim, size, hps, device, final=True)
            
            elif datainfo.name in ['imnet64', 'celeba']:
                # SCALE 4: 24 x 8 x 8
                self.s4_ckbd = self.checkerboard_combo(chan, dim, size, hps, device)
                self.s4_chan = self.channelwise_combo(chan*4, dim*2, hps, device)
                ...
                # SCALE 5: 48 x 4 x 4
                self.s5_ckbd = self.checkerboard_combo(chan, dim, size, hps, device, final=True)

    def checkerboard_combo(self, in_out_dim, mid_dim, size, hps, device, final=False):
        ...
        if final:
            return nn.ModuleList([
                CheckerboardCoupling(in_out_dim, mid_dim, size, 1., hps, device),
                CheckerboardCoupling(in_out_dim, mid_dim, size, 0., hps, device),
                CheckerboardCoupling(in_out_dim, mid_dim, size, 1., hps, device),
                CheckerboardCoupling(in_out_dim, mid_dim, size, 0., hps, device)])
        else:
            return nn.ModuleList([
    ...
    def channelwise_combo(self, in_out_dim, mid_dim, hps, device):
        ...
        """
        return nn.ModuleList([
                ChannelwiseCoupling(in_out_dim, mid_dim, 0., hps, device),
                ChannelwiseCoupling(in_out_dim, mid_dim, 1., hps, device),
                ChannelwiseCoupling(in_out_dim, mid_dim, 0., hps, device)])
